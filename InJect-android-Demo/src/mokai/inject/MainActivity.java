...
public class MainActivity extends Activity {
	...
	@ResourceInject(R.drawable.ic_launcher)//图片等
	private Drawable icon;
	
	@ResourceInject(R.array.str_arr)//string数组
	private String[] stringArray;
	
	@ResourceInject(R.array.int_arr)//int数组
	private int[] intArray;
	
	//Animation
	private Animation anima;
	
	@ResourceInject(R.layout.activity_main)//layout
	private View view;
	
	private ColorStateList colorStateList;//color state list
	
	// 需要注意的是,在定义dimens为整型(int)时，我们这里必须通过type为进行区别,浮点型就不需要
	@ResourceInject(value = R.dimen.app_width,type = ResourceType.COLOR)//dimens
	private int appWidth; 
	@ResourceInject(R.dimen.app_width)
	private float appWidthF; 
	@ResourceInject(value = R.color.app_color,type = ResourceType.COLOR)//color
	...
	public void click(View view){
		...
		new ViewIOC(R.id.class).setFormat("user_%s").setViewFilter(new InjectFilter() {
